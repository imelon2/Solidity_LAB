...
const walletShipper = new ethers.Wallet(process.env.SHIPPER_KEY,provider);
const walletCarrier = new ethers.Wallet(process.env.CARRIER_KEY,provider);
const walletForwarder = new ethers.Wallet(process.env.ADMIN_KEY,provider);
...
async function fwCreateOrder() {
  ...
  //2770 data 생성
  const req = await sigMaker.forwardData(
  ...
  const sig = await forwarderContract.signMetaTx(
    req,
    walletShipper.connect(provider)
  );
...
async function fwSelectOrder(_orderId: number, reward: BigNumber) {
  ...
  const permitDataCarrier = await sigMaker.permitCarrierData(
    orderId,
    walletCarrier.address
  ); // 캐리어의 담보 서명
  ...
  await _DKA
    .connect(walletShipper)
    .transfer(
      walletForwarder.address,
  ...
  const selectData = await orderContract.selectOrder(
    orderId,
    orderData,
    await walletCarrier._signTypedData(
      format.order_domain((await provider.getNetwork()).chainId, CA.OrderCA),
      format.order_types(),
      orderData
    ),
    permitDataCarrier,
    await walletCarrier._signTypedData(
      format.permit_domain((await provider.getNetwork()).chainId, CA.DKA),
      format.permit_types(),
      permitDataCarrier
    ),
    permitDataShipper,
    await walletShipper._signTypedData(
      format.permit_domain((await provider.getNetwork()).chainId, CA.DKA),
      format.permit_types(),
      permitDataShipper
    )
...
async function fwPickOrderWithOutSig(orderId: number) {
  ...
  const sig = await forwarderContract.signMetaTx(
    req,
    walletCarrier.connect(provider)
  );
  await forwarderContract.execute(req, sig, walletSigner);
}
...
async function fwCancelOrderByFaultShipper(orderId: number) {
  //취소 당사자(화주)의 정보 + 서명
  const orderData = await orderContract.signatureData(
    orderId,
    walletShipper.address
  );
  const cancelSig = await walletShipper._signTypedData(
    format.order_domain((await provider.getNetwork()).chainId, CA.OrderCA),
    format.order_types(),
    orderData
  );

  const cancelData = await orderContract.cancelOrderByFault(
    orderId,
    walletShipper.address,
    cancelSig
  );
...
async function fwCancelOrderByFaultCarrier(orderId: number) {
  ...
  const cancelSig = await walletCarrier._signTypedData(
    format.order_domain((await provider.getNetwork()).chainId, CA.OrderCA),
    format.order_types(),
    orderData
  );
...
(async () => {
  ...
  try {
    await fwCreateOrder();
    await fwSelectOrder(orderId, ethers.utils.parseEther("98"));
    console.log("SUCCESS SelectOrder");
  } catch (error) {
